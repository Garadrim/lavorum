			...
			@if (!empty($latest_posts[0]))
				@foreach ($latest_posts as $latest_post)
					<div class="table3">
						<div>
							<div>
								<a href="{{ url('/lavorum/view/'.$latest_post->slug) }}">{{ str_limit($latest_post->title, $limit = 10, $end = '...') }}</a>
							</div>
						</div>
						<div>
			...
			@if(!empty($latest_comments[0]))
				@foreach($latest_comments as $latest_comment)
					<div class="list-group-item">
						<div class="table2b">
							<div>
								<a href="{{ url('/lavorum/view/'.$latest_comment->post->slug) }}">{{ str_limit($latest_comment->post->title, $limit = 10, $end = '...') }}</a>
							</div>
